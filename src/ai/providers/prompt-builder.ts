...
- UNITY FUND: the Unity Fund is the ORGANIZATION-level reserve, NOT a member account. The single source of truth is UnityFundEngine (src/lib/services/unity-fund.engine.ts). Its actual balance = actual receipts (received/posted org inflows) - posted expenditures. ACTUAL sources: contributions (contribution_monthly/special/development transactions), welfare (welfare_deposit), fines (fine_payment), registration/annual fees, loan interest (loan_interest_receipts, separated from principal), donations.received_amount, grants.received_amount, organization_loans.received_amount. PENDING receivables (member_financial_obligations + pledged donations + approved-but-unreceived grants + accrued unpaid loan interest) are NEVER added to the actual balance — they are reported separately. ORGANIZATION LOANS received are cash AND a liability (outstanding_liability = received - repaid), NEVER income/profit. LOAN INTEREST is org money: on repayment, principal stays on the member/loan account and only the interest portion is routed to the Unity Fund (loan_interest_receipts), so a member's personal position is never inflated by interest. Expenditures (unity_fund_expenditures) require authorization and must not exceed available actual cash. The reconciliation status (unity_fund_reconciliation_runs) compares the engine ledger vs the DB view vs the source-breakdown sum vs the dashboard path; any divergence is a confirmed finding. The Unity Fund endpoints are GET /api/v1/unity-fund/{summary,balance,pending,transactions,sources,expenditures,liabilities,reconciliation} and POST /api/v1/unity-fund/{expenditures,donations,grants,organization-loans}. There is NO direct balance-edit endpoint.
=== END STORAGE MODEL ===
