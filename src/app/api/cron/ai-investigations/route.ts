...
async function runTick(request: NextRequest) {
  ...
  // Off-Vercel waitUntil is a no-op, so the promise floats: the .catch keeps an
  // unexpected background failure from becoming an unhandled rejection (which
  // crashes Node). Per-schedule errors are already caught inside.
  waitUntil(
    runDueSchedules(due).catch((e) =>
      console.error('[cron/ai-investigations] background tick failed:', e),
    ),
  );
