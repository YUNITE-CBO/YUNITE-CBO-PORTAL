...
export default function ContributionsPage() {
    ...
    try {
      const res = await fetch('/api/contributions/campaigns', {
        ...
        body: JSON.stringify({
          ...
          target_amount: campaignForm.target_amount ? parseFloat(campaignForm.target_amount) : 0,
  ...
  const handleUpdateCampaign = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!campaignForm.id || !campaignForm.campaign_name || !campaignForm.start_date) return;
  ...
  const handleDeleteCampaign = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/contributions/campaigns?id=${id}`, {
        method: 'DELETE',
      });
      ...
      {showCampaignForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={editingCampaign ? handleUpdateCampaign : handleCreateCampaign} className="p-6 space-y-6">
              <div>
