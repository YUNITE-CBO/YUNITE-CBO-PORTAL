...
SELECT
    ...
    m.email,
    m.phone,
    COALESCE(cs.value::NUMERIC, 1000)::NUMERIC(15,2) AS amount_due,
    COALESCE(paid.this_month_paid, 0)::NUMERIC(15,2) AS amount_paid,
    GREATEST(COALESCE(cs.value::NUMERIC, 1000) - COALESCE(paid.this_month_paid, 0), 0)::NUMERIC(15,2) AS remaining,
...
SELECT
    ...
    m.email,
    m.phone,
    COALESCE(ws.value::NUMERIC, 500)::NUMERIC(15,2) AS amount_due,
    COALESCE(paid.this_month_paid, 0)::NUMERIC(15,2) AS amount_paid,
    GREATEST(COALESCE(ws.value::NUMERIC, 500) - COALESCE(paid.this_month_paid, 0), 0)::NUMERIC(15,2) AS remaining,
    last_day_of_month() AS due_date,
    last_day_of_month() AS repayment_end_date,
    'monthly'::TEXT AS source_status,
    CASE
